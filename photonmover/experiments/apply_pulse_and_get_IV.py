# Applies pulses of specified duration and amplitude (with the HP pulse generator and electrical attenuator) 
...
class apply_pulse_and_get_IV(Experiment):
    ...
    def check_necessary_instruments(self, instrument_list):
        ...
        if (self.smu is not None) and (self.el_att is not None) and (self.patt_gen is not None):
            return True
        else:
            return False
    ...
    def get_description(self):
        ...
        """
        return """ Applies pulses of a given duration and amplitude and then acquires an IV curve. """
    ...
    def perform_experiment(self, params, filename=None):
        ...
        iv_biases = params["iv_voltages"]  # List of voltages at which we want to get the IV curve
        pulse_durations = params["pulse_durations"]  # List of durations for each of the pulses (in s)
        pulse_amps = params["pulse_amps"]  # List of pulse amplitudes (in V)
        puls_appl_t = params["pulse_appl_t"]  # How long should we apply the pulse train (in s)
        puls_amp_0dB_att = params["pulse_amp_0dB_att"]  # This is the amplitude of the pulse generated by the setup with 0 dB attenuation

        # For each combination of pulse amplitude and pulse duration, apply pulses for the specified duration and
        ...
        for pulse_amp in pulse_amps:
            ...
            # Set the correct pulse amplitude. We modulate the pulse amplitude by controlling the attenuation of the electrical attenuator.

            if pulse_amp > puls_amp_0dB_att:
                print('The specified amplitude is not achievable with this setup! Doing max amplitude')
                ...
                real_pulse_amp = puls_amp_0dB_att*np.power(10, -power_att/20)
            
            for pulse_duration in pulse_durations:

                # Configure the pattern generator to generate the desired pulses
                self.patt_gen.configure_pulse(repetition_rate=35e6, pulse_width=pulse_duration,
                                              amplitude=0.9, hilevel=0.9, polarity='positive') # Hardcoded values!
                ...
                n_filename = '%s--pulse_total_time=%.2fs--pulse_width=%.2es--pulse_amp=%.2fV' % (filename, puls_appl_t, pulse_duration, real_pulse_amp)
                iv_data = iv_acq.perform_experiment(params={"voltages":iv_biases}, filename=n_filename)
                self.data = iv_data

                print('Done amp = %.2fV; duration = %.2es' % (real_pulse_amp, pulse_duration))
    ...
    def required_params(self):
        ...
        """
        return ["iv_voltages", "pulse_durations", "pulse_amps", "pulse_appl_t", "pulse_amp_0dB_att"]
...
if __name__ == '__main__':
    ...
    #p_widths = [100e-12, 200e-12, 300e-12, 400e-12, 500e-12, 600e-12, 700e-12, 800e-12, 900e-12, 1000e-12]
    ...
    params = {"iv_voltages": iv_volts, "pulse_durations": p_widths, "pulse_amps": p_amps, "pulse_appl_t": p_app_t,
