...
class Calibration(Experiment):
    ...
    def get_description(self):
        ...
        """
        return """ Sweeps wavelength, measures transmission. Meant for calibration of the ratio between the measured tap power and the launched power."""
    ...
    def calibration_daq(self, wavs):
        ...
        for w in wavs:
            ...
            self.daq.configure_nsampl_acq([AIN_RECEIVED, AIN_TAP], clk_channel=None, num_points=MEAS_COUNT)
            ...
            for i in range(len(daq_data[0])):
                ratios.append(daq_data[1][i]/daq_data[0][i])  # tap_power / received_power
    ...
    def plot_data(self, canvas_handle, data=None):
        ...
        plot_graph(x_data=wavs, y_data=meas, canvas_handle=canvas_handle, xlabel='Wavelength (nm)', ylabel='Tap to received power ratio', title='Calibration', legend=None)