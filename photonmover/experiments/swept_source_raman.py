...
class SweptSourceRaman(Experiment):
    ...
    def check_necessary_instruments(self, instrument_list):
        ...
        if (self.laser is not None) and (self.daq is not None) and (self.optical_switch is not None) and (self.power_meter is not None):
            return True
        else:
            return False
    ...
    def perform_experiment(self, params, filename=None):
        ...
        wavs = params["wavs"]
        channels = params["channels"] # Channels to measure (of the Optical switch)
        ...
        sampling_freq = params["sampling_freq"] # Sampling frequency of the DAQ
        ...
        self.daq.task.timing.cfg_samp_clk_timing(sampling_freq, samps_per_chan=num_samples)
        ...
        for wav in wavs:
            ...
            while (not success) and num_trials < MAX_NUM_WAV_SET_TRIALS:
                ...
                success = self.laser.set_wavelength(wav) # returns 1 if successful
                time.sleep(1.0)
                measured_wavelength = self.laser.get_wavelength()

                if abs(measured_wavelength-wav) > 0.01: 
                    # using just wavelength setting in laser but may need to implement wavemeter           
                    success = 0

                num_trials = num_trials + 1

            if num_trials == MAX_NUM_WAV_SET_TRIALS:
                print('We could not correctly set to a %.3f nm wavelength. Skipping it' % wav)
                continue
            ...
            for channel in channels:
                ...
                power_output_dic[channel] = list()  # time domain power monitoring
                ...
                time.sleep(0.5)
                print('Measuring on channel %d' % self.optical_switch.get_channel())
                ...
                for n in range (1, num_reps+1):
                    ...
                    if self.daq is not None: 
                        ...
                        self.daq.start_task()
                        self.daq.wait_task(timeout = num_samples/sampling_freq*1.5)                 
                    ...
                    ni_output_dic[channel].append([measured_wavelength] + list(buff_ni))
                ...
                if filename is not None:
                    ...
                    nidaq_file_name = "SSR-nidaq-%s-channel=%d--%d#%d#%d_%d#%d#%d.csv" % (filename,
                                                                                          ...
                                                                                          time_tuple[4],
                                                                                          time_tuple[5])
                    np.savetxt(nidaq_file_name, 
                                np.vstack((time_array_save, np.array(ni_output_dic[channel]))).T,
                                delimiter=",",fmt='%s')
    ...
    def required_params(self):
        ...
        """
        return ["wavs", "channels", "int_time", "num_reps", "lock-in", "sampling_freq"]
...
if __name__ == '__main__':
    ...
    daq.configure_channel_acq(["cDAQ1Mod1/ai0", "cDAQ1Mod1/ai1"], min_vals = [0.0, 0.0], max_vals = [10.0, 2.5])
    ...
    params = {"wavs": wavs, "channels": channels, "int_time": int_time, "num_reps": num_reps, "lock-in": False, "sampling_freq": 1000}
