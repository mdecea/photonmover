...
class ramanmover_GUI(QtGui.QMainWindow):
    ...
    def __init__(self, instr_list):
        ...
        # turningg the laser on and off, setting SMU voltage/current, TEC temperature...
        ...
        self.instr_worker.finished.connect(self.__disconnect_instr_thread_start_events__)
        self.instr_worker.stats_vals.connect(self.refresh_GUI_stats)  # Update the stats when we have the info
    ...
    def _create_indicators_(self):
        ...
        if "wavs" in self.param_list:
            ...
            self.laser_power_button = QtGui.QPushButton("Toggle Laser State", self) 
        ...
        if "powers" in self.param_list:
            ...
            self.indicator_param_map["powers"] = ([self.init_p, self.end_p, self.num_p], 'textbox')
        ...
        if "sampling_freq" in self.param_list:
            ...
            self.indicator_param_map["sampling_freq"] = ([self.sampling_freq], 'textbox')
        ...
        self.title = self.plot_area.setTitle('Power (DAQ) vs Time', size='20pt')
        ...
        self.ylabel = self.plot_area.setLabel('left',text='Voltage (V)', **self.label_styles)
        font=QtGui.QFont()
    ...
    def show_about(self):
        ...
        layout.addWidget(QtGui.QLabel(""" This is ramanmover. GUI for Raman-related experiments. Written in python 3 and using 
    ...
    def __disconnect_instr_thread_start_events__(self):
        ...
        We need this so we can tie different methods to the start of the thred (depending on the
    ...
    def laser_on(self):
        ...
        try:
            ...
            self.instr_worker_thread.started.connect(lambda: self.instr_worker.laser_on(op))
    ...
    def set_laser_wav(self):
        ...
        try: 
            ...
            self.instr_worker_thread.started.connect(
                lambda: self.instr_worker.set_laser_wav(set_wav, self.tf_with_laser.isChecked()))
            self.instr_worker_thread.start()
    ...
    def set_laser_power(self):
        ...
        try: 
            ...
            self.instr_worker_thread.started.connect(lambda: self.instr_worker.set_laser_power(set_power))
            self.instr_worker_thread.start()
    ...
    def set_pm_range(self):
        ...
        try:
            ...
            self.instr_worker_thread.started.connect(lambda: self.instr_worker.set_pm_range(set_range))
            self.instr_worker_thread.start()
    ...
    def call_experiment(self, experiment_object):
        ...
        if filename:
            self.exp_thread.started.connect(lambda: experiment_object.run_experiment(params, filename) )
    ...
    def refresh_stats(self):
        ...
        self.instr_worker_thread.started.connect(lambda: self.instr_worker.get_raman_stats(DAQ_PLOT_CHANNEL) )
        self.instr_worker_thread.start()
