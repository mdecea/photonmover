...
class Experiment(QObject, ABC, metaclass=FinalMeta):
    ...
    # The signal needs to be created this way for some reason. This is used for threading.
    finished = pyqtSignal()
    ...
    def run_experiment(self, params, filename=None):
        """
        Calls perform_experiment of the object and takes care of the threading stuff.
        """
    ...
    def perform_experiment(self, params, filename=None):
        ...
        :return: All the relevant data to higher level experiments that call
                a lower level experiment. For example, a "Tx vs V" sweep uses the simple "Tx" sweep. The "Tx" sweep
                returns in the first element all the data sent to the Tx vs V.
        """
        pass
    ...
    def check_all_params(self, params):
        """
        This function checks that all required parameters are present. If a given parameter is not present, it checks if it is given as a 
        ...
        for p in req_params:
            ...
            if p not in params:
                ...
                if p in self.default_params():
                    ...
                    raise ValueError('Param %s missing in list of provided parameter, and there is no default provided.' % p) 

            # THIS IS EXPERIMENTAL. Is meant to handle parameters that are dictionnaries. In this case, we might have specified some of 
            # the keys but left some other unspecified. With this, we are trying to fill this missing keys if they are specified in the default params.
    ...
    def plot_data(self, canvas_handle, data=None):
        """
        This method plots the data gathered by the experiment in the canvas given as a parameter
        :param canvas_handle: where to plot the data
        :param data: the data to plot. This can be None, in which case what is plotted is the last result of the experiment (which is saved internally).
            Of course, this will throw an error if plot_data is called before having called perform_experiment (unless data is not None)
        """
        pass