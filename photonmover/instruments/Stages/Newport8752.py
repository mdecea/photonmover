...
class Newport8752(Instrument):
    ...
    def __init__(self, axis_map={'x': 'a1', 'y': 'a2'}, units={'x':1/30e-9, 'y':1/30e-9}):
        super().__init__()

        self.axis_map = axis_map  # Axis map indicating which driver (a1 - a8) corresponds to each axis
        
        self.wait = 0.1

        self.units = units  # Converts between encoder positions and a given distance unit. 
                # For example, if 1 um = 10000 encoder steps, then we could do units={'x':1000, 'y':1000}
                # And then, if we call move(units=1), we will move 1 um (which is equal to 1000 encoder steps)
    ...
    def initialize(self):
        ...
        self.serial = serial.Serial(COM_ADDRESS, baudrate=19200, bytesize=serial.EIGHTBITS, 
                   parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, 
                   timeout=3, xonxoff=True, rtscts=False, dsrdtr=False)
    ...
    def move_to_limit(self, axis, direction='forward', vel=None, acc=None, motor=0):
        """
    ...
    def move_steps(self, steps, axis, vel=None, acc=None, motor=0, go=True):
        ...
        self.set_axis(axis, vel=vel, acc=acc, motor=motor)
        cmd = 'rel {driver}={steps}'.format(driver=self.axis_map[axis], steps=steps)
        if go:
            cmd = cmd + ' g'
        return self.sendrecv(cmd)
    ...
    def move(self, units, axis, vel=None, acc=None, motor=0, go=True):
        ...
        return self.move_steps(steps, axis, vel=None, acc=None, motor=0, go=True)
