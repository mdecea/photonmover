...
class ThorlabsCamera(Instrument, Camera):
    ...
    def get_frame(self, filename):
        ...
        file_props = IMAGE_FILE_PARAMS(filename,  file_ext_indicator, 0, None, None)
        success_save = self.thorlabs_dll.is_ImageFile(self.handle, IS_IMAGE_FILE_CMD_SAVE, byref(file_props), sizeof(file_props))
    ...
    def initialize_memory(self):
        if self.meminfo != None:
            self.thorlabs_dll.is_FreeImageMem(self.handle, self.meminfo[0], self.meminfo[1])
        ...
        self.thorlabs_dll.is_SetAllocatedImageMem(self.handle, xdim, ydim, 8, c_buf, byref(memid))
        self.thorlabs_dll.is_SetImageMem(self.handle, c_buf, memid)
        self.meminfo = [c_buf, memid]

    def connect(self, bit_depth=8, roi_shape=(1024, 1024), roi_pos=(0,0), exposure = 20.0, frametime = 340.0):
        ...
        if i == 0:
            ...
            self.thorlabs_dll.is_SetColorMode(self.handle, 6) # 6 is for monochrome 8 bit. See uc480.h for definitions
