...
class LFI3751(TempController,Instrument):
    ...
    def initialize(self):
        ...
        try:
            ...
            raise ValueError('Cannot connect to Wavelength Electronics temp controller')
    ...
    def set_temperature(self, temperature):
        ...
        if np.abs(temperature) > 199.999:
            raise ValueError( "Temperature setpoint must be within +/- 199.999 C" )

        self.tec.query( self._format_msg(cmd='write', code=3, value=temperature))
    ...
    def set_sensor_properties(self, sensor_type="thermistor", sensor_parameters=None):
        """
        ...
        if sensor_type == "thermistor":
            ...
            self.tec.query( self._format_msg(cmd='write', code=41, value=-0.010))

            # Set A/B/C parameters
            if not set(sensor_parameters.keys()) == set(['A1', 'A2', 'B1', 'B2', 'C1', 'C2']):
                raise ValueError( 'The sensor_parameters dict must contain ONLY A1, A2, B1, B2, C1, and C2 keyword/value pairs.' )

            code_dict = {'A1': 21, 'A2': 22,'B1': 23,'B2': 24,'C1': 25,'C2': 26,}
            for k,v in sensor_parameters.items():
                self.tec.query( self._format_msg(cmd='write', code=code_dict[k], value=v) )
    ...
    def set_temperature_limits( self, upper=None, lower=None ):
        ...
        if upper is None:
            self.tec.query( self._format_msg(cmd='write', code=31, value=199.999))
        elif upper is not None:
            self.tec.query( self._format_msg(cmd='write', code=31, value=upper))

        if lower is None:
            self.tec.query( self._format_msg(cmd='write', code=32, value=-199.999))
    ...
    def _format_msg(self, cmd, code, value ):
        ...
        msg = '!1{:02d}{:1d}{:02d}{:+08.3f}'.format(self.rs232_address, cmd, code, value)
