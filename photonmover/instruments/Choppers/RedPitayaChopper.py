...
class RedPitayaChopper(Instrument):
    ...
    def initialize(self):
        ...
        try:
            ...
            self.rpc_s.update(
                {self.hostname: scpi.scpi(self.ipc_s[self.hostname])})
        except BaseException:
            print(
                '{} is unavailable: {}'.format(
                    self.hostname,
                    sys.exc_info()))
        else:
            print('Chopper RedPitaya at {} has IP {} is connected'.format(
                self.hostname, self.ipc_s[self.hostname]))

    def close(self):
        try:
            # Turn off MEMS chopper
            self.rpc_s[self.hostname].tx_txt('OUTPUT1:STATE OFF')
            print('{} with IP {} successfully disconnected'.format(
                self.hostname, self.ipc_s[self.hostname]))
        except BaseException:
            # hostname, sys.exc_info()))
            print('Could not close redpitayas : {}'.format(sys.exc_info()))
    ...
    def configure_modulation(self, shape, shape_params):
        ...
        if shape == 'SQUARE':
            ...
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:FUNC %s' % (self.outp_channel, shape.upper()))
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:FREQ:FIX %.2f' % (self.outp_channel, freq))
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:VOLT %.2f' % (self.outp_channel, Vpp))
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:VOLT:OFFS %.2f' % (self.outp_channel, Voffset))
        ...
        if shape == 'ARBITRARY':
            ...
            # The value of each bit as a fraction of Vpp (i.e, 0.5 means that
            # the bit is Voffset + 0.5*Vpp)
            code = shape_params["code"]
            # 1/code_freq is the time for each code
            code_freq = shape_params["code_freq"]
            Vpp = shape_params["Vpp"]
            Voffset = shape_params["Voffset"]

            # Only relevant in BURST mode
            # Number of periods in a burst
            cyc_in_burst = shape_params["cyc_in_burst"]
            ...
            # generate data to load RedPitaya by expanding each code to mult
            # length
            code_buff = []
            for v in code:
            ...
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:FUNC %s' % (self.outp_channel, shape.upper()))
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:BURS:STAT %s' % (self.outp_channel, burst_mode.upper()))
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:TRIG:IMM' % self.outp_channel)

            if burst_mode == 'CONTINUOUS':
                self.rpc_s[self.hostname].tx_txt(
                    'SOUR%d:FREQ:FIX ' % (self.outp_channel, freq))
            elif burst_mode == 'BURST':
                self.rpc_s[self.hostname].tx_txt(
                    'SOUR%d:BURS:NCYC %d' % (self.outp_channel, cyc_in_burst))
                self.rpc_s[self.hostname].tx_txt(
                    'SOUR%d:BURS:NOR %d' % (self.outp_channel, reps))

                usec_per_bit = int(sec_per_bit * 1.0e6)
                self.rpc_s[self.hostname].tx_txt(
                    'SOUR%d:BURS:INT:PER %d' % (self.outp_channel, usec_per_bit))
            ...
            self.rpc_s[self.hostname].tx_txt(
                'SOUR%d:TRAC:DATA:DATA %s' % (self.outp_channel, code_str))

        else:

            raise ValueError(
                'The indicated shape for the chopper is not suppoerted/recognized')

    def turn_on(self):
        self.rpc_s[self.hostname].tx_txt(
            'OUTPUT{}:STATE ON'.format(self.outp_channel))

    def turn_off(self):
        self.rpc_s[self.hostname].tx_txt(
            'OUTPUT{}:STATE OFF'.format(self.outp_channel))
